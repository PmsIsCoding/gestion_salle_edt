...
$getSeances = "SELECT e.*, m.*, u.*,c.*
    FROM emploisdutemps e
    JOIN modules m ON e.module_id = m.module_id
    JOIN utilisateurs u ON m.professeur_id = u.utilisateur_id
    JOIN classes c ON e.classe_id = c.classe_id
    WHERE e.salle_id = ?
";
$seances = dbGetter( $getSeances,array($_GET['salle']));

// var_dump($seances);

$getNom = "SELECT nom_salle FROM salles WHERE salle_id = ?";
$nomSalle = dbGetter($getNom,array($_GET['salle']));
    ...
    foreach($seances as $row) : 
        if($row['jour'] == $jour && $row['heure_debut'] == $debut && $row['heure_fin'] == $fin){
            $node = "<div class=\"emploi modaleSrc\" data-id='".$row['emploi_id']."' data-debut=".$debut." data-fin=".$fin." data-jour=".$jour." data-option='Attribution' data-seance=".$row["emploi_id"]."><span class='nom'>".
                ...
                $row['nom_utilisateur']."</span><br><span class='classe'>".
                $row['nom_classe']."</span>
                </div>";
        }
    endforeach;
...
<body>
    ...
    <main>
    <main>
        <h1>Emplois du Temps de <span><?php echo $nomSalle[0]['nom_salle'] ?></span></h1>
        <table>
            <tr>
